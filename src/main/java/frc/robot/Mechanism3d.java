...
public class Mechanism3d {
  ...
  public void log() {
    ...
    Rotation2d intakeAngle = deploySubsystem.getPosition();
    Rotation2d turretAngle = turretSubsystem.getTurretAngleRobotRelative();

    Pose3d turretPose = Robot.isSimulation() ? 
    ...
    Pose3d hoodPose =
        turretPose.transformBy(
            new Transform3d(
                HoodConstants.turretToHood, new Rotation3d(0, Robot.isSimulation() ? hoodSubsystem.getSimPos().getRadians() : hoodAngle.getRadians(), 0)));
